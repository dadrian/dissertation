...
TLS also specifies a mode of using Diffie-Hellman with fixed parameters from the server's certificate~\cite{rfc3279}. This mode is not forward secret, was never widely adopted, and has been removed from all modern browsers due to dangerous protocol flaws~\cite{kcitls}. The only widely used form of Diffie-Hellman in TLS today is ephemeral Diffie-Hellman, described above.
...
Delignat-Lavaud~\cite{conf/ndss/BhargavanDP15}. The attacker modifies the
...
queries to learn the  server's secret $x_s \bmod q_i$~\cite{Lim1997}. This
...
any remaining bits of the exponent~\cite{Lim1997}.



We note that the TLS False Start extension allows the
server to send application data before receiving the client's authentication~\cite{tls-falsestart}.
...
%\todo{Add discussion of the old unused TLS extension that makes this offline. \ref{tls-falsestart}}
...
on key management~\cite{nist-sp-800-57-rev2,nist-sp-800-57-rev4} to determine secret exponent lengths from the length of the prime.~\cite{nss-line-of-code}  Thus NSS uses 160-bit exponents with a 1024-bit prime, and 224-bit exponents with a 2048-bit prime.
...
factorization~\cite{odlyzko95}. Botan, a C++ cryptography and TLS library,
...
We used ZMap~\cite{zmap} to probe the public IPv4 address space for hosts
...
Dovecot~\cite{mail-imc15}) that use these primes follow recommendations to use
...
static Diffie-Hellman~\cite{kcitls}, and Censys shows no hosts with DSS
