...
reject short groups.\looseness=-1
...
for the Internet community.\looseness=-1
...
There are two reasons for this. First, a surprising number of servers
...
\subpar{Active attacks on export ciphers in TLS\@.}  We introduce Logjam, a new
...
\subpar{Risks from common 1024-bit groups.} We explore the implications
...
\subpar{Mitigations and lessons.} In response to the Logjam attack, mainstream
...
\begin{table}[t]
	...
	\caption{\textbf{Top 512-bit DH primes for TLS\@.}  8.4\% of
      ...
      one of the two most popular primes, shown here.\looseness=-1}
...
%\subpar{Export-grade Diffie-Hellman.}
...
Given the widespread use of these primes, an attacker with the ability
...
% candidate to not be dropped depending on how much space we have in the end
...
%For the 768-bit case,
%we based our initial estimates on the recent discrete log
%record at 596 bits~\cite{dlp180} and the integer factorization record
%of 768 bits from 2009~\cite{rsa768}.  While the algorithms for
%factorization and discrete log are similar, the discrete log linear
%algebra stage is many times more difficult.
%
%A 596-bit factorization takes about 5 core-years, most of it spent on
%sieving.  In comparison, the record 596-bit discrete log effort tuned
%parameters such that they spent 50 core-years on sieving. This reduced
%their linear algebra calculation to 80 core-years.  We used this same
%strategy in our 512-bit experiments
%in~\S\ref{subsec:512bit-dl-computation}.
%
...
%was around 1500 core-years and the matrix that was produced had 200M
%rows and columns. As a result, the linear algebra took 150 core-years.
%Taking algorithmic improvements since 2009 into account and
%optimizing for the total time, we estimate that factoring an
%RSA-768 integer would take 900 core-years in total.
%
%For a 768-bit discrete log, we can expect ten times as much
%sieving as the RSA case.  We extrapolate from experiments with existing software and estimate that
%this linear algebra would take 28,500 core-years, for
%a total of 36,500 core-years. This is within reach by computing power
%available to academics.
%
%The descent step takes relatively little time.  We experimented with
%both CADO-NFS and a new implementation with GMP-ECM based on
%the early-abort strategy~\cite{batchnfs} finding that  the initial descent phase took an average of around
%1~core-day. The remaining phase uses sieving similar to the  precomputation and, extrapolating from experiments, the rest of
%the descent should take at most 1~core-day. After
%precomputation, the cost of a single 768-bit discrete log computation
%is around 2~core-days.\footnote{Subsequent research found that
%these initial estimates may have been substantial over-estimates. Kleinjung
%et~al.\  performed the computation against a 768-bit safe prime finding that the
%total effort required an order of magnitude less computation. Their
%precomputation required the equivalent of about 5,300~core years (approximately
%% PZ: this is 200 and not 90: see Table 1 of [15]: 110 core years for
%% |g_max| <= 165, and 90 core years for |g_max| <= 333
%200~for polynomial selection, 4,000~for sieving,
%% PZ: 4000 is just for sieving, linear algebra took 920 core years (Table 1)
%and 920 for linear algebra) and
%% PZ: it is 3-220~core hours not years! Cf Table 1 of [15]
%3-220~core hours (average of 43) for descent~\cite{Kleinjung2017}.}
%
...
% The time complexity for each individual log after the precomputation
% should be multiplied by
% $L_{2^{1024}}(1.206)/L_{2^{768}}(1.206)\approx86$, where $k=1.206$
% follows from~\cite{EC:FGHT17}.
% This last number does not correspond to what
% we observed in practice and we attribute that to the fact that the descent
% step has been far less studied.
...
To put this dollar figure in context, the FY\,2012 budget for the
...
The IKE protocol has been extensively analyzed~\cite{ck02,meadows99}
...
In this section, we use Internet-wide scanning to assess the impact of
...
small, widely shared groups.\looseness=-1
